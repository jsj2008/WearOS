//
//  NSString+XMLEntities.m
//
//
//  Created by Michael Waterfall on 11/05/2010.
//  Copyright 2010 d3i. All rights reserved.
//

#import "NSString+XMLEntities.h"

@implementation NSString (XMLEntities)

#pragma mark -
#pragma mark Class Methods

#pragma mark -
#pragma mark Instance Methods

- (NSString*) stringByDecodingXMLEntities {
	// Find first & and short-cut if we can
	NSUInteger ampIndex = [self rangeOfString: @"&" options: NSLiteralSearch].location;

	if (ampIndex == NSNotFound) {
		return [NSString stringWithString: self]; // return copy of string as no & found
	}

	// Make result string with some extra capacity.
	NSMutableString* result = [[NSMutableString alloc] initWithCapacity:(NSUInteger) (self.length * 1.25)];

	// First iteration doesn't need to scan to & since we did that already, but for code simplicity's sake we'll do it again with the scanner.
	NSScanner* scanner = [NSScanner scannerWithString: self];
	[scanner setCharactersToBeSkipped: nil];
	[scanner setCaseSensitive: YES];

	// Boundary characters for scanning unexpected &#... pattern
	NSCharacterSet* boundaryCharacterSet = [NSCharacterSet characterSetWithCharactersInString: @" \t\n\r;"];

	// Scan
	do {
		// Scan up to the next entity or the end of the string.
		NSString* nonEntityString;

		if ([scanner scanUpToString: @"&" intoString: &nonEntityString]) {
			[result appendString: nonEntityString];
		}

		if ([scanner isAtEnd]) break;

		// Common character entity references first
		if ([scanner scanString: @"&amp;" intoString: NULL])
			[result appendString: @"&"];
		else if ([scanner scanString: @"&apos;" intoString: NULL])
			[result appendString: @"'"];
		else if ([scanner scanString: @"&quot;" intoString: NULL])
			[result appendString: @"\""];
		else if ([scanner scanString: @"&lt;" intoString: NULL])
			[result appendString: @"<"];
		else if ([scanner scanString: @"&gt;" intoString: NULL])
			[result appendString: @">"];
		else if ([scanner scanString: @"&nbsp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)160];
		else if ([scanner scanString: @"&laquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)171];
		else if ([scanner scanString: @"&raquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)187];
		else if ([scanner scanString: @"&ndash;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8211];
		else if ([scanner scanString: @"&mdash;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8212];
		else if ([scanner scanString: @"&lsquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8216];
		else if ([scanner scanString: @"&rsquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8217];
		else if ([scanner scanString: @"&ldquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8220];
		else if ([scanner scanString: @"&rdquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8221];
		else if ([scanner scanString: @"&bull;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8226];
		else if ([scanner scanString: @"&hellip;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8230];

		// Numeric character entity references
		else if ([scanner scanString: @"&#" intoString: NULL]) {
			// Entity
			BOOL gotNumber;
			unsigned charCode;
			NSString* xForHex = @"";

			// Is it hex or decimal?
			if ([scanner scanString: @"x" intoString: &xForHex]) {
				gotNumber = [scanner scanHexInt: &charCode];
			} else   {
				gotNumber = [scanner scanInt: (int*) & charCode];
			}

			// Process
			if (gotNumber) {
				// Append character
				[result appendFormat: @"%C", (unsigned short)charCode];
				[scanner scanString: @";" intoString: NULL];
			} else   {
				// Failed to get a number so append to result and log error
				NSString* unknownEntity = @"";
				[scanner scanUpToCharactersFromSet: boundaryCharacterSet intoString: &unknownEntity];
				[result appendFormat: @"&#%@%@", xForHex, unknownEntity]; // Append exact same string
			}

			// Quick check for isolated & with a space after to speed up checks
		} else if ([scanner scanString: @"& " intoString: NULL])
			[result appendString: @"& "];

		// No so common character entity references
		else if ([scanner scanString: @"&iexcl;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)161];
		else if ([scanner scanString: @"&cent;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)162];
		else if ([scanner scanString: @"&pound;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)163];
		else if ([scanner scanString: @"&curren;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)164];
		else if ([scanner scanString: @"&yen;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)165];
		else if ([scanner scanString: @"&brvbar;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)166];
		else if ([scanner scanString: @"&sect;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)167];
		else if ([scanner scanString: @"&uml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)168];
		else if ([scanner scanString: @"&copy;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)169];
		else if ([scanner scanString: @"&ordf;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)170];
		else if ([scanner scanString: @"&not;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)172];
		else if ([scanner scanString: @"&shy;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)173];
		else if ([scanner scanString: @"&reg;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)174];
		else if ([scanner scanString: @"&macr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)175];
		else if ([scanner scanString: @"&deg;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)176];
		else if ([scanner scanString: @"&plusmn;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)177];
		else if ([scanner scanString: @"&sup2;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)178];
		else if ([scanner scanString: @"&sup3;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)179];
		else if ([scanner scanString: @"&acute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)180];
		else if ([scanner scanString: @"&micro;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)181];
		else if ([scanner scanString: @"&para;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)182];
		else if ([scanner scanString: @"&middot;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)183];
		else if ([scanner scanString: @"&cedil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)184];
		else if ([scanner scanString: @"&sup1;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)185];
		else if ([scanner scanString: @"&ordm;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)186];
		else if ([scanner scanString: @"&frac14;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)188];
		else if ([scanner scanString: @"&frac12;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)189];
		else if ([scanner scanString: @"&frac34;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)190];
		else if ([scanner scanString: @"&iquest;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)191];
		else if ([scanner scanString: @"&Agrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)192];
		else if ([scanner scanString: @"&Aacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)193];
		else if ([scanner scanString: @"&Acirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)194];
		else if ([scanner scanString: @"&Atilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)195];
		else if ([scanner scanString: @"&Auml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)196];
		else if ([scanner scanString: @"&Aring;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)197];
		else if ([scanner scanString: @"&AElig;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)198];
		else if ([scanner scanString: @"&Ccedil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)199];
		else if ([scanner scanString: @"&Egrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)200];
		else if ([scanner scanString: @"&Eacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)201];
		else if ([scanner scanString: @"&Ecirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)202];
		else if ([scanner scanString: @"&Euml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)203];
		else if ([scanner scanString: @"&Igrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)204];
		else if ([scanner scanString: @"&Iacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)205];
		else if ([scanner scanString: @"&Icirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)206];
		else if ([scanner scanString: @"&Iuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)207];
		else if ([scanner scanString: @"&ETH;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)208];
		else if ([scanner scanString: @"&Ntilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)209];
		else if ([scanner scanString: @"&Ograve;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)210];
		else if ([scanner scanString: @"&Oacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)211];
		else if ([scanner scanString: @"&Ocirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)212];
		else if ([scanner scanString: @"&Otilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)213];
		else if ([scanner scanString: @"&Ouml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)214];
		else if ([scanner scanString: @"&times;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)215];
		else if ([scanner scanString: @"&Oslash;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)216];
		else if ([scanner scanString: @"&Ugrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)217];
		else if ([scanner scanString: @"&Uacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)218];
		else if ([scanner scanString: @"&Ucirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)219];
		else if ([scanner scanString: @"&Uuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)220];
		else if ([scanner scanString: @"&Yacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)221];
		else if ([scanner scanString: @"&THORN;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)222];
		else if ([scanner scanString: @"&szlig;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)223];
		else if ([scanner scanString: @"&agrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)224];
		else if ([scanner scanString: @"&aacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)225];
		else if ([scanner scanString: @"&acirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)226];
		else if ([scanner scanString: @"&atilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)227];
		else if ([scanner scanString: @"&auml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)228];
		else if ([scanner scanString: @"&aring;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)229];
		else if ([scanner scanString: @"&aelig;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)230];
		else if ([scanner scanString: @"&ccedil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)231];
		else if ([scanner scanString: @"&egrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)232];
		else if ([scanner scanString: @"&eacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)233];
		else if ([scanner scanString: @"&ecirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)234];
		else if ([scanner scanString: @"&euml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)235];
		else if ([scanner scanString: @"&igrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)236];
		else if ([scanner scanString: @"&iacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)237];
		else if ([scanner scanString: @"&icirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)238];
		else if ([scanner scanString: @"&iuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)239];
		else if ([scanner scanString: @"&eth;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)240];
		else if ([scanner scanString: @"&ntilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)241];
		else if ([scanner scanString: @"&ograve;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)242];
		else if ([scanner scanString: @"&oacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)243];
		else if ([scanner scanString: @"&ocirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)244];
		else if ([scanner scanString: @"&otilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)245];
		else if ([scanner scanString: @"&ouml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)246];
		else if ([scanner scanString: @"&divide;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)247];
		else if ([scanner scanString: @"&oslash;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)248];
		else if ([scanner scanString: @"&ugrave;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)249];
		else if ([scanner scanString: @"&uacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)250];
		else if ([scanner scanString: @"&ucirc;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)251];
		else if ([scanner scanString: @"&uuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)252];
		else if ([scanner scanString: @"&yacute;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)253];
		else if ([scanner scanString: @"&thorn;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)254];
		else if ([scanner scanString: @"&yuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)255];
		else if ([scanner scanString: @"&OElig;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)338];
		else if ([scanner scanString: @"&oelig;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)339];
		else if ([scanner scanString: @"&Scaron;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)352];
		else if ([scanner scanString: @"&scaron;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)353];
		else if ([scanner scanString: @"&Yuml;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)376];
		else if ([scanner scanString: @"&fnof;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)402];
		else if ([scanner scanString: @"&circ;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)710];
		else if ([scanner scanString: @"&tilde;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)732];
		else if ([scanner scanString: @"&Alpha;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)913];
		else if ([scanner scanString: @"&Beta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)914];
		else if ([scanner scanString: @"&Gamma;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)915];
		else if ([scanner scanString: @"&Delta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)916];
		else if ([scanner scanString: @"&Epsilon;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)917];
		else if ([scanner scanString: @"&Zeta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)918];
		else if ([scanner scanString: @"&Eta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)919];
		else if ([scanner scanString: @"&Theta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)920];
		else if ([scanner scanString: @"&Iota;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)921];
		else if ([scanner scanString: @"&Kappa;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)922];
		else if ([scanner scanString: @"&Lambda;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)923];
		else if ([scanner scanString: @"&Mu;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)924];
		else if ([scanner scanString: @"&Nu;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)925];
		else if ([scanner scanString: @"&Xi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)926];
		else if ([scanner scanString: @"&Omicron;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)927];
		else if ([scanner scanString: @"&Pi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)928];
		else if ([scanner scanString: @"&Rho;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)929];
		else if ([scanner scanString: @"&Sigma;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)931];
		else if ([scanner scanString: @"&Tau;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)932];
		else if ([scanner scanString: @"&Upsilon;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)933];
		else if ([scanner scanString: @"&Phi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)934];
		else if ([scanner scanString: @"&Chi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)935];
		else if ([scanner scanString: @"&Psi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)936];
		else if ([scanner scanString: @"&Omega;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)937];
		else if ([scanner scanString: @"&alpha;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)945];
		else if ([scanner scanString: @"&beta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)946];
		else if ([scanner scanString: @"&gamma;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)947];
		else if ([scanner scanString: @"&delta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)948];
		else if ([scanner scanString: @"&epsilon;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)949];
		else if ([scanner scanString: @"&zeta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)950];
		else if ([scanner scanString: @"&eta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)951];
		else if ([scanner scanString: @"&theta;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)952];
		else if ([scanner scanString: @"&iota;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)953];
		else if ([scanner scanString: @"&kappa;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)954];
		else if ([scanner scanString: @"&lambda;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)955];
		else if ([scanner scanString: @"&mu;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)956];
		else if ([scanner scanString: @"&nu;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)957];
		else if ([scanner scanString: @"&xi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)958];
		else if ([scanner scanString: @"&omicron;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)959];
		else if ([scanner scanString: @"&pi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)960];
		else if ([scanner scanString: @"&rho;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)961];
		else if ([scanner scanString: @"&sigmaf;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)962];
		else if ([scanner scanString: @"&sigma;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)963];
		else if ([scanner scanString: @"&tau;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)964];
		else if ([scanner scanString: @"&upsilon;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)965];
		else if ([scanner scanString: @"&phi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)966];
		else if ([scanner scanString: @"&chi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)967];
		else if ([scanner scanString: @"&psi;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)968];
		else if ([scanner scanString: @"&omega;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)969];
		else if ([scanner scanString: @"&thetasym;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)977];
		else if ([scanner scanString: @"&upsih;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)978];
		else if ([scanner scanString: @"&piv;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)982];
		else if ([scanner scanString: @"&ensp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8194];
		else if ([scanner scanString: @"&emsp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8195];
		else if ([scanner scanString: @"&thinsp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8201];
		else if ([scanner scanString: @"&zwnj;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8204];
		else if ([scanner scanString: @"&zwj;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8205];
		else if ([scanner scanString: @"&lrm;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8206];
		else if ([scanner scanString: @"&rlm;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8207];
		else if ([scanner scanString: @"&sbquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8218];
		else if ([scanner scanString: @"&bdquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8222];
		else if ([scanner scanString: @"&dagger;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8224];
		else if ([scanner scanString: @"&Dagger;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8225];
		else if ([scanner scanString: @"&permil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8240];
		else if ([scanner scanString: @"&prime;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8242];
		else if ([scanner scanString: @"&Prime;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8243];
		else if ([scanner scanString: @"&lsaquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8249];
		else if ([scanner scanString: @"&rsaquo;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8250];
		else if ([scanner scanString: @"&oline;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8254];
		else if ([scanner scanString: @"&frasl;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8260];
		else if ([scanner scanString: @"&euro;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8364];
		else if ([scanner scanString: @"&image;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8465];
		else if ([scanner scanString: @"&weierp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8472];
		else if ([scanner scanString: @"&real;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8476];
		else if ([scanner scanString: @"&trade;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8482];
		else if ([scanner scanString: @"&alefsym;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8501];
		else if ([scanner scanString: @"&larr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8592];
		else if ([scanner scanString: @"&uarr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8593];
		else if ([scanner scanString: @"&rarr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8594];
		else if ([scanner scanString: @"&darr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8595];
		else if ([scanner scanString: @"&harr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8596];
		else if ([scanner scanString: @"&crarr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8629];
		else if ([scanner scanString: @"&lArr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8656];
		else if ([scanner scanString: @"&uArr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8657];
		else if ([scanner scanString: @"&rArr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8658];
		else if ([scanner scanString: @"&dArr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8659];
		else if ([scanner scanString: @"&hArr;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8660];
		else if ([scanner scanString: @"&forall;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8704];
		else if ([scanner scanString: @"&part;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8706];
		else if ([scanner scanString: @"&exist;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8707];
		else if ([scanner scanString: @"&empty;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8709];
		else if ([scanner scanString: @"&nabla;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8711];
		else if ([scanner scanString: @"&isin;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8712];
		else if ([scanner scanString: @"&notin;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8713];
		else if ([scanner scanString: @"&ni;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8715];
		else if ([scanner scanString: @"&prod;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8719];
		else if ([scanner scanString: @"&sum;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8721];
		else if ([scanner scanString: @"&minus;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8722];
		else if ([scanner scanString: @"&lowast;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8727];
		else if ([scanner scanString: @"&radic;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8730];
		else if ([scanner scanString: @"&prop;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8733];
		else if ([scanner scanString: @"&infin;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8734];
		else if ([scanner scanString: @"&ang;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8736];
		else if ([scanner scanString: @"&and;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8743];
		else if ([scanner scanString: @"&or;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8744];
		else if ([scanner scanString: @"&cap;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8745];
		else if ([scanner scanString: @"&cup;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8746];
		else if ([scanner scanString: @"&int;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8747];
		else if ([scanner scanString: @"&there4;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8756];
		else if ([scanner scanString: @"&sim;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8764];
		else if ([scanner scanString: @"&cong;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8773];
		else if ([scanner scanString: @"&asymp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8776];
		else if ([scanner scanString: @"&ne;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8800];
		else if ([scanner scanString: @"&equiv;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8801];
		else if ([scanner scanString: @"&le;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8804];
		else if ([scanner scanString: @"&ge;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8805];
		else if ([scanner scanString: @"&sub;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8834];
		else if ([scanner scanString: @"&sup;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8835];
		else if ([scanner scanString: @"&nsub;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8836];
		else if ([scanner scanString: @"&sube;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8838];
		else if ([scanner scanString: @"&supe;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8839];
		else if ([scanner scanString: @"&oplus;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8853];
		else if ([scanner scanString: @"&otimes;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8855];
		else if ([scanner scanString: @"&perp;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8869];
		else if ([scanner scanString: @"&sdot;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8901];
		else if ([scanner scanString: @"&lceil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8968];
		else if ([scanner scanString: @"&rceil;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8969];
		else if ([scanner scanString: @"&lfloor;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8970];
		else if ([scanner scanString: @"&rfloor;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)8971];
		else if ([scanner scanString: @"&lang;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9001];
		else if ([scanner scanString: @"&rang;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9002];
		else if ([scanner scanString: @"&loz;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9674];
		else if ([scanner scanString: @"&spades;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9824];
		else if ([scanner scanString: @"&clubs;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9827];
		else if ([scanner scanString: @"&hearts;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9829];
		else if ([scanner scanString: @"&diams;" intoString: NULL])
			[result appendFormat: @"%C", (unsigned short)9830];
		else {
			// Must be an isolated & with no space after
			NSString* amp;
			[scanner scanString: @"&" intoString: &amp]; // isolated & symbol
			[result appendString: amp];
		}
	}
	while (![scanner isAtEnd]);

	// Finish
	NSString* resultingString = [NSString stringWithString: result];
	return resultingString;
}

// Needs more work to encode more entities
- (NSString*) stringByEncodingXMLEntities {
	// Scanner
	NSScanner* scanner = [[NSScanner alloc] initWithString: self];
	[scanner setCharactersToBeSkipped: nil];
	NSMutableString* result = [NSMutableString new];
	NSString* temp;
	NSCharacterSet* characters = [NSCharacterSet characterSetWithCharactersInString: @"&\"'<>"];
	[scanner setCharactersToBeSkipped: nil];

	// Scan
	while (![scanner isAtEnd]) {
		// Get non new line or whitespace characters
		temp = nil;
		[scanner scanUpToCharactersFromSet: characters intoString: &temp];

		if (temp) [result appendString: temp];

		// Replace with encoded entities
		if ([scanner scanString: @"&" intoString: NULL])
			[result appendString: @"&amp;"];
		else if ([scanner scanString: @"'" intoString: NULL])
			[result appendString: @"&apos;"];
		else if ([scanner scanString: @"\"" intoString: NULL])
			[result appendString: @"&quot;"];
		else if ([scanner scanString: @"<" intoString: NULL])
			[result appendString: @"&lt;"];
		else if ([scanner scanString: @">" intoString: NULL])
			[result appendString: @"&gt;"];
	}

	// Return
	NSString* retString = [NSString stringWithString: result];
	return retString;
}

- (NSString*) stringByStrippingTags {
	// Find first & and short-cut if we can
	NSUInteger ampIndex = [self rangeOfString: @"<" options: NSLiteralSearch].location;

	if (ampIndex == NSNotFound) {
		return [NSString stringWithString: self]; // return copy of string as no tags found
	}

	// Scan and find all tags
	NSScanner* scanner = [NSScanner scannerWithString: self];
	[scanner setCharactersToBeSkipped: nil];
	NSMutableSet* tags = [NSMutableSet new];
	NSString* tag;

	do {
		// Scan up to <
		tag = nil;
		[scanner scanUpToString: @"<" intoString: NULL];
		[scanner scanUpToString: @">" intoString: &tag];

		// Add to set
		if (tag) {
			NSString* t = [[NSString alloc] initWithFormat: @"%@>", tag];
			[tags addObject: t];
		}
	}
	while (![scanner isAtEnd]);

	// Strings
	NSMutableString* result = [[NSMutableString alloc] initWithString: self];
	NSString* finalString;

	// Replace tags
	NSString* replacement;

	for (NSString* t in tags) {
		// Replace tag with space unless it's an inline element
		replacement = @" ";

		if ([t isEqualToString: @"<a>"] ||
		    [t isEqualToString: @"</a>"] ||
		    [t isEqualToString: @"<span>"] ||
		    [t isEqualToString: @"</span>"] ||
		    [t isEqualToString: @"<strong>"] ||
		    [t isEqualToString: @"</strong>"] ||
		    [t isEqualToString: @"<em>"] ||
		    [t isEqualToString: @"</em>"]) {
			replacement = @"";
		}

		// Replace
		[result replaceOccurrencesOfString: t
		 withString: replacement
		 options: NSLiteralSearch
		 range: NSMakeRange(0, result.length)];
	}

	// Remove multi-spaces and line breaks
	finalString = [result stringByRemovingNewLinesAndWhitespace];

	return finalString;
}

- (NSString*) stringWithNewLinesAsBRs {
	// Strange New lines:
	//	Next Line, U+0085
	//	Form Feed, U+000C
	//	Line Separator, U+2028
	//	Paragraph Separator, U+2029

	// Scanner
	NSScanner* scanner = [[NSScanner alloc] initWithString: self];
	[scanner setCharactersToBeSkipped: nil];
	NSMutableString* result = [NSMutableString new];
	NSString* temp;
	NSCharacterSet* newLineCharacters = [NSCharacterSet characterSetWithCharactersInString:
	                                     [NSString stringWithFormat: @"\n\r%C%C%C%C", (unsigned short)0x0085, (unsigned short)0x000C, (unsigned short)0x2028, (unsigned short)0x2029]];

	// Scan
	do {
		// Get non new line characters
		temp = nil;
		[scanner scanUpToCharactersFromSet: newLineCharacters intoString: &temp];

		if (temp) [result appendString: temp];

		temp = nil;

		// Add <br /> s
		if ([scanner scanString: @"\r\n" intoString: nil]) {
			// Combine \r\n into just 1 <br />
			[result appendString: @"<br />"];
		} else if ([scanner scanCharactersFromSet: newLineCharacters intoString: &temp])   {
			// Scan other new line characters and add <br /> s
			if (temp) {
				for (int i = 0; i < temp.length; i++) {
					[result appendString: @"<br />"];
				}
			}
		}
	}
	while (![scanner isAtEnd]);

	NSString* retString = [NSString stringWithString: result];

	return retString;
}

- (NSString*) stringByRemovingNewLinesAndWhitespace {
	// Strange New lines:
	//	Next Line, U+0085
	//	Form Feed, U+000C
	//	Line Separator, U+2028
	//	Paragraph Separator, U+2029

	// Scanner
	NSScanner* scanner = [[NSScanner alloc] initWithString: self];
	[scanner setCharactersToBeSkipped: nil];
	NSMutableString* result = [NSMutableString new];
	NSString* temp;
	NSCharacterSet* newLineAndWhitespaceCharacters = [NSCharacterSet characterSetWithCharactersInString:
	                                                  [NSString stringWithFormat: @" \t\n\r%C%C%C%C", (unsigned short)0x0085, (unsigned short)0x000C, (unsigned short)0x2028, (unsigned short)0x2029]];

	// Scan
	while (![scanner isAtEnd]) {
		// Get non new line or whitespace characters
		temp = nil;
		[scanner scanUpToCharactersFromSet: newLineAndWhitespaceCharacters intoString: &temp];

		if (temp) [result appendString: temp];

		// Replace with a space
		if ([scanner scanCharactersFromSet: newLineAndWhitespaceCharacters intoString: NULL]) {
			if (result.length > 0 && ![scanner isAtEnd]) // Dont append space to beginning or end of result
				[result appendString: @" "];
		}
	}

	// Return
	NSString* retString = [NSString stringWithString: result];
	return retString;
}

@end