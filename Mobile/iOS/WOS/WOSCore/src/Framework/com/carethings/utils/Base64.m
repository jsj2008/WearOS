...
+ (void) initialize {
	if (self == [Base64 class]) {
		...
		for (NSInteger i = 0; i < ArrayLength(encodingTable); i++) {
			decodingTable[encodingTable[i]] = (char) i;
		}
	}
}
...
+ (NSString*) encode:(const uint8_t*) input length:(NSInteger) length {
    ...
    for (NSInteger i = 0; i < length; i += 3) {
        ...
        output[index + 0] = (uint8_t) encodingTable[(value >> 18) & 0x3F];
        output[index + 1] = (uint8_t) encodingTable[(value >> 12) & 0x3F];
        output[index + 2] = (uint8_t) ((i + 1) < length ? encodingTable[(value >> 6)  & 0x3F] : '=');
        output[index + 3] = (uint8_t) ((i + 2) < length ? encodingTable[(value >> 0)  & 0x3F] : '=');
    }
...
+ (NSData*) decode:(const char*) string length:(NSInteger) inputLength {
    ...
    NSMutableData *data;
    data = [NSMutableData dataWithLength:(NSUInteger) outputLength];
