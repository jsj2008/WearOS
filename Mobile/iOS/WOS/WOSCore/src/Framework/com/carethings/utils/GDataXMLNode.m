...
static const void *StringCacheKeyRetainCallBack(CFAllocatorRef __unused allocator, const void *str);

static void StringCacheKeyReleaseCallBack(CFAllocatorRef __unused allocator, const void *str);
...
+ (GDataXMLElement *)elementWithName:(NSString *)name {
    ...
    if (theNewNode) {
        // succeeded
        return (GDataXMLElement *) [self nodeConsumingXMLNode:theNewNode];
    }
...
+ (GDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
    ...
    return theNewNode ? (GDataXMLElement *) [self nodeConsumingXMLNode:theNewNode] :nil;
...
- (void)setTagString:(NSString *)tagVal {
    tagString = tagVal;
}
...
- (NSString *)stringFromXMLString:(const xmlChar *)chars {
    ...
    if (xmlNode_ != NULL && (xmlNode_->type == XML_ELEMENT_NODE
        ...
        if (xmlNode_->doc != NULL) {
            ...
            if (cacheDict) {
                ...
                result = (__bridge NSString *) CFDictionaryGetValue(cacheDict, (void *) chars);
    ...
    if (cacheDict) {
        ...
        CFDictionarySetValue(cacheDict, (void *) chars, (void *) (__bridge const void *)(result));
    }
...
- (NSString *)XMLString {
    ...
    if (xmlNode_ != NULL) {
        ...
        if (buff) {
            ...
            if (result > -1) {
                str = [[NSString alloc] initWithBytes:(void *) (xmlBufferContent(buff))
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"%@ %p: {type:%d name:%@ xml:\"%@\"}",
                                      [self class], (__bridge void *) self, nodeType, [self name], [self XMLString]];
}
...
- (void)setNamespaces:(NSArray *)namespaces {
    if (xmlNode_ != NULL) {
        ...
        [[self class] fixUpNamespacesForNode:xmlNode_ graftingToTreeNode:xmlNode_];
    }
}
    ...
    if (xmlNode_ != NULL) {
        ...
        if (childNodeCopy) {
            ...
            if (resultNode == NULL) {
                ...
                [[self class] fixUpNamespacesForNode:childNodeCopy graftingToTreeNode:xmlNode_];
            }
        }
    }
}

- (void)addChildren:(GDataXMLNode *)parent {
    NSArray* childrenEnum = [parent children];

    for (GDataXMLElement* childElement in childrenEnum) {
        ...
        for (GDataXMLNode *child in children) {
            ...
            if (currChildPtr->type == XML_ELEMENT_NODE) {
                ...
                if (currChildPtr->ns == childLocalNS
                        && currChildPtr->name != NULL && xmlStrEqual(currChildPtr->name, childDesiredLocalName)) {
                    if (array == nil) {
                        array = [NSMutableArray arrayWithObject:child];
                    } else {
                        [array addObject:child];
        ...
        if (attrPtr) {
            ...
            if (oldAttr == NULL) {
                ...
                if (newProp != NULL) {
                    ...
                    [[self class] fixUpNamespacesForNode:(xmlNodePtr) newProp graftingToTreeNode:xmlNode_];
                }
...
- (GDataXMLNode *)attributeForName:(NSString *)name {
    if (xmlNode_ != NULL) {
        ...
        if (attrPtr == NULL) {
            ...
            const xmlChar *nsURI = ((ns != NULL) ? (void *) ns->href : NULL);
...
+ (void)fixDuplicateNamespacesForNode:(xmlNodePtr)nodeToFix
    ...
    if (nodeToFix->type == XML_ELEMENT_NODE) {
        ...
        while (definedNS != NULL) {
            ...
            if (foundNS != NULL && foundNS != definedNS
...
- (id)initWithData:(NSData *)data options:(__unused unsigned int)mask error:(NSError **)error {
    self = [super init];
...
- (void)addStringsCacheToDoc {
    ...
    CFDictionaryKeyCallBacks keyCallBacks = {
            0, // version
            (CFDictionaryRetainCallBack) StringCacheKeyRetainCallBack,
            (CFDictionaryReleaseCallBack) StringCacheKeyReleaseCallBack,
            (CFDictionaryCopyDescriptionCallBack) StringCacheKeyCopyDescriptionCallBack,
            (CFDictionaryEqualCallBack) StringCacheKeyEqualCallBack,
            (CFDictionaryHashCallBack) StringCacheKeyHashCallBack
    };
